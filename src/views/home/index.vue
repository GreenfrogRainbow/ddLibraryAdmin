<template>
	<div class="home-container layout-pd">
		...
		<el-row :gutter="15" class="home-card-three">
			<el-col :xs="24" :sm="10" :md="10" :lg="8" :xl="8" class="home-media">
				<div class="home-card-item">
					<div style="height: 100%" ref="borrowPieRef"></div>
				</div>
			</el-col>
...
const textarea = ref('')
const saveToStore = (content) => {
	memoStore.setContent(content)
}
...
const initLineChart = async () => {
	...
	let list = []
	for(let i=1; i< 13 ;i++){
		const res = await request.get(`/admin/getClickCount?id=${i}`)
		list.push(res.data.cnt)
	}
	const option = {
		...
		legend: { data: ['实际统计量', '预计统计量'], right: 0 },
		xAxis: {
		...
		series: [
			...
			},
			{
				...
				symbol: 'circle',
				smooth: true,
				data: [20000, 25000, 20000, 30000, 40000, 45000, 50000, 50000, 50000, 50000, 55000, 55000],
...
const initPieChart = async () => {
	const res = await request.get('/admin/getTalkTagsCount')
	const list_help = res.data
	let listSum = list_help.reduce((sum, item) => {
		sum += item.cnt
		return sum
	}, 0)
	...
	var data = [];
	var getname = []
	for (var i = 0; i < list_help.length; i++) {
		getname[i] = list_help[i].tag
		list_help[i].cnt = ((list_help[i].cnt / listSum) * 100).toFixed(0)
		data.push({ name: list_help[i].tag, value: list_help[i].cnt });
	}
	const colorList = ['#51A3FC', '#36C78B', '#FEC279', '#968AF5', '#E790E8', '#16779B'];
	const option = {
		backgroundColor: state.charts.bgColor,
		title: {
			text: '讨论热度',
...
const initBorrowPieChart = async () => {
	const res = await request.get('/admin/getBorrowTagsCount')
	console.log(res)
	const list_help = res.data
	let listSum = list_help.reduce((sum, item) => {
		sum += item.cnt
	...
	state.global.homeChartFour = markRaw(echarts.init(borrowPieRef.value, state.charts.theme));
	var data = [];
	var getname = []
	for (var i = 0; i < list_help.length; i++) {
		getname[i] = list_help[i].tag
...
const getInfos = async ()=> {
	const res1 = await request.get('/admin/getBooksCount')
	state.homeOneOne[0].num1 = res1.data.cnt
	const res2 = await request.get('/admin/getBorrowCount')
	state.homeOneOne[1].num1 = res2.data.cnt
	const res3 = await request.get('/admin/getTalkCount')
	state.homeOneOne[2].num1 = res3.data.cnt
	state.homeOneOne[3].num1 = 0
	for(let i=1;i < 13; i++) {
		const res4 = await request.get(`/admin/getClickCount?id=${i}`)
		state.homeOneOne[3].num1 += res4.data.cnt
	}
...
// 页面加载时
onMounted(() => {
	textarea.value = memoStore.content
	getInfos()
	initEchartsResize();
});
// 由于页面缓存原因，keep-alive
