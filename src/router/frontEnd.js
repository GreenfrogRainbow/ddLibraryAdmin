...
export async function initFrontEndControlRoutes() {
	...
	if (useUserInfo().userRole.roles.length <= 0) return Promise.resolve(true);
	// 添加动态路由
	await setAddRoute();
...
export function setFilterRoute(chil) {
	...
	const { userRole } = storeToRefs(stores);
	let filterRoute = [];
	chil.forEach((route) => {
		if (route.meta.roles) {
			route.meta.roles.forEach((metaRoles) => {
				userRole.value.roles.forEach((roles) => {
...
export function setCacheTagsViewRoutes() {
	...
	const { userRole } = storeToRefs(stores);
	let rolesRoutes = setFilterHasRolesMenu(dynamicRoutes, userRole.value.roles);
...
export function setFilterMenuAndCacheTagsViewRoutes() {
	...
	const { userRole } = storeToRefs(stores);
	storesRoutesList.setRoutesList(setFilterHasRolesMenu(dynamicRoutes[0].children, userRole.value.roles));
